...
\begin{itemize}
    \item \textbf{L1.} The length of the shortest augmenting path never
    decreases. When we augment along a shortest path, any new edge added to
    the residual graph goes \emph{backwards}, which can only increase the
    shortest path length.
    \item \textbf{L2.} After at most $m$ augmentations at the same length,
    the shortest path strictly increases. Each augmentation saturates at
    least one edge (the bottleneck), which is then removed from $L_G$. No
    new edge is added to $L_G$ until the length increases.
\end{itemize}

\begin{theorem}
The shortest augmenting path algorithm runs in $O(m^2 n)$ time: $O(m+n)$
per BFS, $O(m)$ augmentations at each of the $O(n)$ possible path lengths.
\end{theorem}
...
\subsection{Perfect Matching and Hall's Theorem}
...
Two paths are \textbf{edge-disjoint} if they share no edge. The problem is:
